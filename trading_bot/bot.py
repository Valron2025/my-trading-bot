...
# Импорты для фундаментального анализа
...
class TradingBot:
    ...
    def __init__(self):
        ...
        # ========== ИНИЦИАЛИЗАЦИЯ ФУНДАМЕНТАЛЬНЫХ МОДУЛЕЙ (ПРАВИЛЬНЫЙ ПОРЯДОК) ==========

        # 1. Сначала создаём анализатор
        try:
            from .analysis.fundamental_analyzer import FundamentalAnalyzer
            self.fundamental_analyzer = FundamentalAnalyzer()
            info("✅ FundamentalAnalyzer инициализирован")
        except ImportError as e:
            warning(f"⚠️ Модуль фундаментального анализа не найден: {e}")
            self.fundamental_analyzer = None
        except Exception as e:
            warning(f"⚠️ Ошибка инициализации FundamentalAnalyzer: {e}")
            self.fundamental_analyzer = None
        ...
        try:
            ...
            info("✅ FundamentalDatabase инициализирована")
        except ImportError as e:
            warning(f"⚠️ Модуль БД не найден: {e}")
            self.fundamental_db = None
        except Exception as e:
            warning(f"⚠️ Ошибка инициализации FundamentalDatabase: {e}")
            self.fundamental_db = None

        # 3. Затем создаём апдейтер (использует анализатор и БД)
        try:
            from .analysis.fundamental_updater import FundamentalUpdater
            ...
            info("✅ FundamentalUpdater инициализирован")
        except ImportError as e:
            warning(f"⚠️ Модуль апдейтера не найден: {e}")
            self.fundamental_updater = None
        except Exception as e:
            warning(f"⚠️ Ошибка инициализации FundamentalUpdater: {e}")
            self.fundamental_updater = None

        # Инициализация опциональных модулей (НЕ включая фундаментальные)
        self._init_optional_modules()

        # ========== ВЫЗОВ ИНИЦИАЛИЗАЦИИ ПРОДВИНУТЫХ МЕНЕДЖЕРОВ ==========
    ...
    def init_advanced_managers(self):
        ...
        info("✅ Advanced managers initialized (IcebergManager, TrailingStopManager, PreMarketTrader)")

    async def start_fundamental_updater(self):
        """Запуск автоматического обновления фундаментальных данных"""
        if hasattr(self, 'fundamental_updater') and self.fundamental_updater:
            asyncio.create_task(self.fundamental_updater.start_scheduler(interval_hours=24))
            info("📊 Фундаментальный апдейтер запущен (обновление раз в 24 часа)")

    async def get_fundamental_signal_enhanced(self, ticker: str, technical_score: int):
        """Улучшенный сигнал с учётом исторических трендов"""
        if not hasattr(self, 'fundamental_analyzer') or not self.fundamental_analyzer:
            return technical_score, [], {}

        metrics = await self.fundamental_analyzer.fetch_metrics(ticker)

        if not metrics:
            return technical_score, [], {}

        trend = self.fundamental_db.get_trend(ticker) if hasattr(self, 'fundamental_db') else {'trend': 'stable'}

        impact = self.fundamental_analyzer.SCORE_IMPACT.get(metrics.recommendation[0], 0)

        if trend.get('trend') == 'improving' and impact > 0:
            impact += 1
        elif trend.get('trend') == 'deteriorating' and impact < 0:
            impact -= 1

        new_score = technical_score + impact

        signals = []
        for reason in self.fundamental_analyzer._generate_reasons(metrics, metrics.recommendation[0]):
            signals.append(f"📊 {reason}")

        if trend.get('trend') == 'improving':
            signals.append(f"📈 Фундаментальный тренд: улучшается (+1)")
        elif trend.get('trend') == 'deteriorating':
            signals.append(f"📉 Фундаментальный тренд: ухудшается (-1)")

        return new_score, signals, metrics.to_dict()

    def get_fundamental_report(self, ticker: str = None) -> Dict:
        """Получение отчёта по фундаментальным данным"""
        report = {
            'stats': self.fundamental_updater.stats if hasattr(self, 'fundamental_updater') else {},
            'current_multipliers': self.fundamental_db.get_all_current_multipliers() if hasattr(self, 'fundamental_db') else {},
            'trends': {}
        }

        if ticker:
            report['history'] = self.fundamental_db.get_history(ticker, days=30) if hasattr(self, 'fundamental_db') else []
            report['trend'] = self.fundamental_db.get_trend(ticker) if hasattr(self, 'fundamental_db') else {}
    ...
    def _init_optional_modules(self):
        """Инициализация опциональных модулей (только один раз глобально) - БЕЗ ФУНДАМЕНТАЛЬНЫХ"""
    ...
    def start(self):
        ...
        # # Запуск обновления фундаментальных данных
        # try:
        #     if hasattr(self, 'fundamental_updater') and self.fundamental_updater:
        #         import asyncio
        #         import threading
        ...
        #                 loop = asyncio.new_event_loop()
        #                 asyncio.set_event_loop(loop)
        #                 loop.run_until_complete(self.start_fundamental_updater())
        #                 info("📊 Фундаментальный апдейтер запущен")
        #             except Exception as e:
        #                 warning(f"⚠️ Ошибка запуска фундаментального апдейтера: {e}")
        #             finally:
        #                 loop.close()
        #
