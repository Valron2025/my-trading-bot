...
class MoexAPIClient:
    ...
    async def get_candles(
        ...
        """
        Получение исторических свечей с MOEX

        Args:
            ticker: Тикер инструмента
            interval: Интервал свечи
            days: Количество дней (если не указаны start_date/end_date)
            start_date: Начальная дата
            end_date: Конечная дата
            market: Рынок (shares, bonds, futures, etc.)
            board: Торговая доска
        """
        if not ticker or not ticker.strip():
            error("❌ Empty ticker provided")
            return []
        ...
        try:
            ...
            # Проверяем интервал
            moex_interval = self.INTERVAL_MAP.get(interval)
            if not moex_interval:
                warning(f"⚠️ Unknown interval '{interval}', using default '1day'")
                moex_interval = "24"

            # Формируем запрос
            url = f"/engines/stock/markets/{market}/boards/{board}/securities/{ticker}/candles.json"
            params = {
            ...
            # ✅ Проверка на None и наличие ключа 'candles'
            if not data or 'candles' not in data:
                debug(f"⚠️ Invalid or empty response from MOEX for {ticker}")
                return []

            # Парсим ответ
            candles_data = data.get('candles', {})
            if not candles_data:
                debug(f"⚠️ No candles data in MOEX response for {ticker}")
                return []

            candles_list = candles_data.get('data', [])
            columns = candles_data.get('columns', [])

            if not candles_list or not columns:
                return []
            ...
            for col in required_cols:
                if col not in col_index:
                    error(f"❌ Missing column '{col}' in MOEX response")
                    return []

            candles = []
            for row in candles_list:
                try:
                    # Парсим дату
                    begin_str = row[col_index['begin']] if len(row) > col_index['begin'] else None
                    if not begin_str:
                        continue

                    if ' ' in str(begin_str):
                        timestamp = datetime.strptime(str(begin_str), "%Y-%m-%d %H:%M:%S")
                    else:
                        timestamp = datetime.strptime(str(begin_str), "%Y-%m-%d")

                    candle = {
                        'open': float(row[col_index['open']]),
                        'high': float(row[col_index['high']]),
                        'low': float(row[col_index['low']]),
                        'close': float(row[col_index['close']]),
                        'volume': float(row[col_index['volume']]),
                    ...
                    debug(f"⚠️ Error parsing MOEX candle: {e}")
                    continue

            debug(f"📊 Retrieved {len(candles)} candles for {ticker} from MOEX")
            return candles

        except Exception as e:
            error(f"❌ Error getting candles from MOEX for {ticker}: {e}")
            return []
...
class CandleBuilder:
    ...
    async def start(self):
        """Запуск CandleBuilder"""
        if self.running:
            warning("⚠️ CandleBuilder already running")
            return

        self.running = True
        self.stats["start_time"] = datetime.now()

        # В тестовом режиме ничего не запускаем
        if self.test_mode:
            info("🧪 Test mode: CandleBuilder started (mock mode)")
            return
    ...
    async def stop(self):
        """Остановка CandleBuilder"""
        if not self.running:
            return

        info("🛑 CandleBuilder stopping...")

        # 1. Снимаем флаг running
        self.running = False
        ...
        # 3. Ждём завершения задач с таймаутом
        if self._tasks:
            try:
        ...
        # 5. Даём время на закрытие соединений
        await asyncio.sleep(0.1)
    ...
    async def get_candles(
        ...
        if use_cache and cache_key in self._candle_cache:
            ...
            if (datetime.now() - timestamp).total_seconds() < cache_ttl:
                ...
                debug(f"📦 Cache hit for {identifier}")
    ...
    async def shutdown(self):
        ...
        # 5. Закрываем MOEX клиент
        if self._moex_client:
            await self._moex_client.close()
            self._moex_client = None

        # 6. Закрываем сессию aiohttp если осталась
        if self._moex_client and self._moex_client._session:
            await self._moex_client._session.close()
