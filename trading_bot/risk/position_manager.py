...
class PositionManager:
    ...
    def _close_position(self, position: Position, current_price: float, reason: str, profit_pct: float):
        """
        Закрытие позиции с ИСПОЛЬЗОВАНИЕМ УМНОГО ЗАКРЫТИЯ И АВТОБЛОКИРОВКИ
        """
        from trading_bot.api.tbank_client import tbank
        ...
        now = datetime.now(MOSCOW_TZ)
        attempt = self._close_attempts.get(position.figi, 0) + 1
        self._close_attempts[position.figi] = attempt
        self._last_close_attempt[position.figi] = now

        if attempt > 5:
            error(f"\n{'💀' * 40}")
            error(f"💀 КРИТИЧЕСКОЕ: {ticker} НЕ ЗАКРЫВАЕТСЯ после {attempt} попыток!")
            error(f"💀 Добавляем {ticker} в чёрный список!")
            error(f"{'💀' * 40}")

            blacklist_manager.add_temporary(ticker, ttl_minutes=120)
            self.remove_position(position.figi)
            self._close_attempts.pop(position.figi, None)
            self._last_close_attempt.pop(position.figi, None)
            return

        info(f"\n{'═' * 60}")
        info(f"🔒 ЗАКРЫТИЕ ПОЗИЦИИ {ticker} (попытка #{attempt})")
        ...
        # ========== 1. ОТМЕНА СТОП-ПРИКАЗОВ ==========
        info(f"📋 Шаг 1/3: Отмена стоп-приказов...")
        self._cancel_stop_orders(position)

        # ========== 2. РАСЧЁТ P&L ==========
        profit_amount = position.current_profit_amount(current_price)
        hold_minutes = position.hold_minutes()
        info(f"📋 Шаг 2/3: P&L = {profit_amount:+.2f}₽ ({profit_pct:+.2f}%), удержание {hold_minutes:.1f} мин")

        # ========== 3. УМНОЕ ЗАКРЫТИЕ ==========
        info(f"📋 Шаг 3/3: Умное закрытие...")
        ...
        if result.get('success'):
            ...
            success(f"   💰 Цена закрытия: {result.get('price', current_price):.2f}₽")
            success(f"   📉 Проскальзывание: {result.get('slippage_pct', 0):.2f}%")
            success(f"   🔄 Попыток: {result.get('attempts', 1)}")
            success(f"{'🎉' * 30}")

            self._send_close_notification(position, profit_pct, profit_amount, ticker, reason)
            self.remove_position(position.figi)
            self._close_attempts.pop(position.figi, None)
            self._last_close_attempt.pop(position.figi, None)
            ...
            error(f"\n{'❌' * 30}")
            error(f"❌ НЕ УДАЛОСЬ ЗАКРЫТЬ {ticker} (попытка #{attempt})!")
            error(f"{'❌' * 30}")

            # Позиция остаётся в менеджере, следующая попытка в следующем цикле
            warning(f"   ⏸️ Позиция {ticker} остаётся в менеджере")
    ...
    def close_position(self, figi: str, quantity: Optional[int] = None) -> bool:
        """Публичный метод для закрытия конкретной позиции с проверкой блокировки"""
        ...
        try:
            ...
            if hasattr(position, 'blocked') and position.blocked:
                ...
                if telegram:
                    telegram.send_error(
                        f"🚨 КРИТИЧЕСКАЯ ОШИБКА!\n{ticker} заблокирован! Закройте вручную через приложение!")
                return False
            ...
            if position.side == OrderSide.SHORT:
                info(f"🟢 Закрытие SHORT {position.ticker}: покупка {quantity} шт")
                order_success = _get_tbank().buy(figi, quantity, use_market=True)
            else:
                info(f"🔴 Закрытие LONG {position.ticker}: продажа {quantity} шт")
                order_success = _get_tbank().sell(figi, quantity, use_market=True)

            if order_success:
                if quantity >= position.quantity:
                    self.remove_position(figi)
                    success(f"✅ Позиция {position.ticker} полностью закрыта")
                else:
                    position.quantity -= quantity
                    success(f"✅ Позиция {position.ticker} частично закрыта: осталось {position.quantity} шт")
                return True
            else:
                error(f"❌ Не удалось закрыть позицию {position.ticker}")
                return False
