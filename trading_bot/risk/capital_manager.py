...
class CapitalManager:
    ...
    def calculate_position_size(self, score: int, volatility: float = 0.01, is_short: bool = False) -> Dict:
        # ✅ ДОБАВИТЬ ПРОВЕРКУ ДЛЯ SHORT СИГНАЛОВ
        if score == 0 or (is_short and score > 0) or (not is_short and score < 0):
            return {
                'size_pct': 0,
                'amount': 0,
                'signal_mult': 0,
                ...
                'vol_mult': 1.0,
                'margin_mult': 1.0,
                'reason': "нет сигнала в нужную сторону"
            }
