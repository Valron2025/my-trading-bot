...
class PrometheusMetrics:
    ...
    def update_cache_metrics(self):
        if USE_UNIFIED_CACHE:
            ...
            for name, stat in stats.items():
                # Обновляем метрики в Prometheus
                self.cache_hits.labels(cache=name).set(stat['hits'])
                self.cache_misses.labels(cache=name).set(stat['misses'])
                self.cache_size.labels(cache=name).set(stat['size'])
    ...
    def update_bot_status(self, running: bool = None, cycle_count: int = None,
        ...
        Args:
            running: Статус работы (True/False)
            cycle_count: Номер цикла (опционально)
            positions: Количество позиций (опционально)
            capital: Капитал (опционально)
        """
        if not self.enabled:
            return
