...
БЕЗОПАСНОСТЬ: Все SQL запросы используют параметризацию (placeholder ?)
...
class FundamentalDatabase:
    ...
    БЕЗОПАСНОСТЬ:
    - Все запросы с пользовательскими данными используют параметризацию (?)
    - CREATE TABLE и CREATE INDEX используют фиксированные имена (безопасно)
    - INSERT с фиксированными значениями безопасен
    """
    ...
    def _init_tables(self):
        ...
        БЕЗОПАСНО: CREATE TABLE, CREATE INDEX и INSERT с фиксированными
        значениями не подвержены SQL инъекциям, так как имена таблиц и
        значения жёстко заданы в коде.
        """
        with sqlite3.connect(self.db_path) as conn:
            ...
            # Индексы (имена фиксированы - безопасно)
            conn.execute("""
                CREATE INDEX IF NOT EXISTS idx_fundamental_ticker 
                ON fundamental_metrics_history(ticker)
            """)
            conn.execute("""
                CREATE INDEX IF NOT EXISTS idx_fundamental_date 
                ON fundamental_metrics_history(fetched_date)
            """)
            conn.execute("""
                CREATE INDEX IF NOT EXISTS idx_fundamental_score 
                ON fundamental_metrics_history(overall_score)
            """)

            # Вставка начальных данных по секторам (значения фиксированы - безопасно)
            conn.execute("""
    ...
    def _validate_ticker(self, ticker: str) -> str:
        """
        Валидация тикера перед использованием в запросах

        Args:
            ticker: Тикер для валидации

        Returns:
            Валидированный тикер

        Raises:
            ValueError: Если тикер некорректен
        """
        if not ticker or not isinstance(ticker, str):
            raise ValueError(f"Некорректный тикер: {ticker}")

        # Ограничиваем длину и удаляем опасные символы
        ticker = ticker.strip().upper()[:self.MAX_TICKER_LENGTH]

        # Разрешены только буквы, цифры, точка и дефис
        import re
        if not re.match(r'^[A-Z0-9.-]+$', ticker):
            raise ValueError(f"Тикер содержит недопустимые символы: {ticker}")
    ...
    def save_metrics(self, ticker: str, metrics: FundamentalMetrics, source: str = "moex_estimated"):
        ...
        БЕЗОПАСНО: Использует параметризованный запрос с placeholder (?)

        Args:
            ticker: Тикер инструмента
            metrics: Объект с фундаментальными метриками
            source: Источник данных (по умолчанию 'moex_estimated')
        """
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            # ✅ ПАРАМЕТРИЗОВАННЫЙ ЗАПРОС для обновления текущих мультипликаторов
            conn.execute("""
    ...
    def get_history(self, ticker: str, days: int = 30) -> List[Dict[str, Any]]:
        ...
        # Ограничиваем days разумным пределом
    ...
    def get_current_multipliers(self, ticker: str) -> Optional[Dict[str, Any]]:
        ...
        Returns:
            Словарь с мультипликаторами или None
        """
        # Валидируем входные данные
        ticker = self._validate_ticker(ticker)
    ...
    def get_all_current_multipliers(self) -> Dict[str, Dict[str, Any]]:
        ...
        БЕЗОПАСНО: SELECT без параметров (нет пользовательского ввода)

        Returns:
            Словарь {ticker: мультипликаторы}
        """
        with sqlite3.connect(self.db_path) as conn:
            conn.row_factory = sqlite3.Row
    ...
    def get_sector_multipliers(self, sector: str) -> Optional[Dict[str, Any]]:
        ...
        Returns:
            Словарь с мультипликаторами сектора или None
        """
        # Валидируем входные данные
        if not sector or not isinstance(sector, str):
            return None

        sector = sector.strip().lower()[:self.MAX_SECTOR_LENGTH]
    ...
    def update_sector_multipliers(self, sector: str, multipliers: Dict[str, float]):
        ...
        Args:
            sector: Название сектора
            multipliers: Словарь с новыми значениями мультипликаторов
        """
        # Валидируем входные данные
        if not sector or not isinstance(sector, str):
            raise ValueError(f"Некорректное имя сектора: {sector}")
    ...
    def get_stale_tickers(self) -> List[str]:
        ...
        Returns:
            Список тикеров с устаревшими данными
        """
        with sqlite3.connect(self.db_path) as conn:
            cursor = conn.execute("""
    ...
    def get_trend(self, ticker: str, metric: str = 'overall_score', days: int = 30) -> Dict[str, Any]:
        ...
        БЕЗОПАСНО: Использует безопасный метод get_history()
        metric проверяется на допустимые значения

        Args:
            ticker: Тикер инструмента
            metric: Название метрики для анализа
            days: Количество дней для анализа

        Returns:
            Словарь с результатами анализа тренда
        """
        # Валидируем metric (white list)
        allowed_metrics = {
            'overall_score', 'value_score', 'quality_score',
            'safety_score', 'liquidity_score', 'pe_ratio',
            'pb_ratio', 'roe', 'dividend_yield'
        }

        if metric not in allowed_metrics:
            debug(f"⚠️ Неизвестная метрика {metric}, используем overall_score")
    ...
    def cleanup_old_records(self, days: int = 365) -> int:
        ...
        Args:
            days: Возраст записей в днях для удаления (по умолчанию 365)
