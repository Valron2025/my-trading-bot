...
class MultiTimeframeAnalyzer:
    ...
    def get_signal(self, candles_by_tf: Dict[str, List]) -> MultiTimeframeSignal:
        """
        Получение сигнала на основе данных с разных таймфреймов
        """
        signals = {}
        strengths = {}
        reasons = []
        ...
        for tf, candles in candles_by_tf.items():
            if candles and len(candles) > 0:
                first = candles[0]
                info(f"🔍 [MTF] Таймфрейм {tf}: тип свечи = {type(first).__name__}")
                if isinstance(first, dict):
                    info(f"   ✅ Это словарь! Ключи: {list(first.keys())[:3]}")
