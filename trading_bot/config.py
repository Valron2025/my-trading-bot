...
class TradingConfig:
    ...
    def get_adaptive_short_threshold(self, capital: float) -> float:
        ...
        # SHORT разрешён только при достаточном капитале и short_allowed
        if not mode['can_trade'] or not mode.get('short_allowed', False):
            return -20  # Никогда не сработает

        # Дополнительная проверка по минимальному капиталу для SHORT
    ...
    def update_short_settings(self, total_capital: float) -> Tuple[bool, str]:
        ...
        short_allowed_by_capital = total_capital >= self.min_capital_for_short  # 7000₽
