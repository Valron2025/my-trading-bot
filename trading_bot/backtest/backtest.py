...
class ProfessionalBacktester:
    ...
    # ========== ГЛОБАЛЬНЫЙ КЭШ НА УРОВНЕ КЛАССА ==========
    ...
    def __init__(self,
        ...
        self.historical_data = None  # ✅ ИСПРАВЛЕНО
    ...
    def load_historical_data(self, ticker: str) -> Optional[pd.DataFrame]:
        ...
        if ticker in self._cached_data:
            print(f"📦 Глобальный кэш: данные для {ticker} уже загружены")
            return self._cached_data[ticker].copy()  # ← возвращаем КОПИЮ!

        # 2. Загружаем данные (оригинальная логика)
        ...
        for interval in [15, 5, 1]:
            ...
            if df is not None and len(df) >= self.warmup_candles + 20:
                ...
                self._cached_data[ticker] = df  # ← сохраняем в ГЛОБАЛЬНЫЙ кэш
                return df.copy()  # ← возвращаем КОПИЮ
    ...
    def clear_cache(cls):
        ...
        cls._cached_data.clear()
        print("🧹 Глобальный кэш данных очищен")

    @classmethod
    def get_cache_stats(cls) -> Dict[str, int]:
        """Статистика кэша"""
        return {
            'cached_tickers': len(cls._cached_data),
            'total_rows': sum(len(df) for df in cls._cached_data.values())
        }
...
class ParameterOptimizer:
    ...
    def optimize(self) -> Dict[str, Any]:
        """Оптимизация параметров с progress bar"""
        ...
        # Подсчёт реальных комбинаций (исключая SL >= TP)
        valid_combos = []
        for risk, conf, tp, sl in product(
            ...
            if sl < tp:  # только валидные комбинации
                valid_combos.append((risk, conf, tp, sl))

        print(f"📊 Валидных комбинаций: {len(valid_combos)} (исключено SL >= TP)\n")

        # Используем tqdm для прогресс-бара
        with tqdm(total=len(valid_combos), desc="🔬 Оптимизация", unit="комб",
                  bar_format="{l_bar}{bar}| {n_fmt}/{total_fmt} [{elapsed}<{remaining}, {rate_fmt}]") as pbar:

            for risk, conf, tp, sl in valid_combos:
                ...
                if 'error' not in result and result.get('total_trades', 0) >= 3:
                    ...
                    if score > best_score:
                        ...
                        # Добавляем информацию о новом лучшем в progress bar
                        pbar.set_postfix_str(
                            f"🏆 {result['total_trades']}сд, WR={result['win_rate']:.0f}%, P={result['total_profit']:+.0f}₽")
                    else:
                        # Тихо обновляем progress bar без лишнего вывода
                        pass
                elif 'error' not in result:
                    # Мало сделок — не влияем на прогресс
                    pass
        ...
        # Вывод лучших результатов после завершения
        print(f"\n{'=' * 70}")
        ...
        if best_result:
            print(f"\n✅ ЛУЧШИЕ ПАРАМЕТРЫ:")
            print(f"   Риск на сделку: {best_result['params']['risk_per_trade'] * 100:.0f}%")
            print(f"   Мин. уверенность: score ≥ {best_result['params']['min_confidence']}")
            print(f"   Тейк-профит: +{best_result['params']['take_profit_pct']:.1f}%")
            print(f"   Стоп-лосс: -{best_result['params']['stop_loss_pct']:.1f}%")
