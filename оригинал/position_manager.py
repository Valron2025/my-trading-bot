...
class PositionManager:
    ...
    def add_position(self, figi: str, quantity: int, price: float, side: OrderSide,
                     ...
                     ticker: str = None) -> Position:  # <-- ДОБАВЬТЕ ПАРАМЕТР ticker
        ...
        info(f"📊 ДОБАВЛЕНА ПОЗИЦИЯ {figi}: {side.value} {quantity}шт по {price:.2f}₽")
        info(
        ...
        if auto_set_stop:
            try:
                ...
                # Проверка: требует ли инструмент подтверждения сделок
                if tbank.is_confirmation_required(figi):
                    success(f"📋 Инструмент {figi} требует подтверждения сделок")
                    success(f"   Используем эмуляцию стоп-приказов (без реальных стопов)")
                ...
                if is_available:
                    ...
                    if stop_success:
                        ...
                        warning(f"   ⚠️ Стоп-лосс не установлен")
                    ...
                    if tp_success:
                        ...
                        warning(f"   ⚠️ Тейк-профит не установлен")
                else:
                    warning(f"   ⚠️ Рынок недоступен: {reason}")
            except Exception as e:
                debug(f"Ошибка автоматической установки стоп-лосса/тейк-профита: {e}")
    ...
    def _close_position(self, position: Position, current_price: float, reason: str, profit_pct: float):
        """Закрытие позиции с проверкой достаточности средств"""
        from trading_bot.api.tbank_client import tbank
        ...
        if position.side == OrderSide.SHORT:
            # Для SHORT нужно купить акции назад
            ...
            if available < buy_back_cost:
                error(f"🚫 НЕДОСТАТОЧНО СРЕДСТВ ДЛЯ ЗАКРЫТИЯ SHORT!")
                error(f"   Нужно: {buy_back_cost:.0f}₽, Доступно: {available:.0f}₽")
                error(f"   Позиция: {position.ticker}, {position.quantity} шт по {current_price:.2f}₽")

                # Пробуем закрыть частично с учётом лота
                max_affordable_qty = int(available * 0.9 / current_price)
                if max_affordable_qty >= lot:
                    # Округляем до кратности лоту
                    ...
                    if new_quantity < position.quantity and new_quantity >= lot:
                        ...
                        if tbank.buy(position.figi, new_quantity):
                            ...
                            return
                    elif new_quantity < lot:
                        warning(
                            f"   ⚠️ Даже 1 лот ({lot} шт) требует {current_price * lot:.0f}₽, а доступно {available:.0f}₽")
                else:
                    error(
                        f"❌ Невозможно закрыть SHORT {position.ticker}: даже {lot} лот требует {current_price * lot:.0f}₽ > {available:.0f}₽")
                    return  # Не закрываем полностью

        # ========== 3. ТОЧНОЕ ОКРУГЛЕНИЕ ЦЕНЫ ==========
        rounded_price = tbank._round_to_min_increment(position.figi, current_price)

        if abs(rounded_price - current_price) > 0.01:
            debug(f"💰 Цена скорректирована: {current_price:.4f} → {rounded_price:.4f}")

        # ========== 4. РАСЧЁТ ПРИБЫЛИ ==========
        profit_amount = position.current_profit_amount(rounded_price)

        # ========== 5. КРАСИВЫЙ ВЫВОД В ЛОГ ==========
        ...
        if position.side == OrderSide.SHORT:
            ...
            # Проверяем кратность лоту перед отправкой
