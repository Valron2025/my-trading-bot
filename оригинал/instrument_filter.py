...
class InstrumentFilter:
    ...
    def filter_candidates(self, candidates: List, is_otc_mode: bool = None) -> List:
        ...
        Args:
            ...
            is_otc_mode: Если True, используем мягкие OTC пороги
        ...
        if config.total_capital < 5000:
            ...
            # Фильтруем только по цене (доступные инструменты)
            filtered = [c for c in candidates if 1 <= c.price <= 2000]
            if filtered:
                return filtered
            # Если ничего не подошло, берём первые 5
            return candidates[:5]
        ...
        try:
            ...
            for cand in candidates:
                ...
                # 1. Проверка качества торговли (чёрный/белый список)
                quality_ok, quality_reason = self.check_trading_quality(ticker)
                if not quality_ok:
                    stats['blacklisted' if 'чёрный' in quality_reason else 'otc'] += 1
                    debug(f"   ⏭️ {ticker}: {quality_reason}")
                    continue
                ...
                # 3. Проверка цены (базовая адекватность)
                ...
                try:
                    candles = _get_candles_sync(ticker, days=5)
                    if candles:
                        is_liquid, liquid_reason = self.check_liquidity_with_thresholds(
                            ticker, candles,
                        ...
                        if not is_liquid:
                            ...
                            debug(f"   ⏭️ {ticker}: низкая ликвидность - {liquid_reason}")
                            continue
                    else:
                        if ticker not in getattr(self, 'whitelist_tickers', []):
                            stats['low_liquidity'] += 1
                            debug(f"   ⏭️ {ticker}: нет данных о ликвидности")
                            continue
        ...
        if stats['passed'] < stats['total']:
            ...
            if stats['blacklisted'] > 0:
                info(f"   ⛔ Чёрный список: {stats['blacklisted']}")
            if stats['otc'] > 0:
            ...
            if stats['passed'] > 0:
                ...
                info(f"   ⚠️ Нет подходящих инструментов!")
        else:
